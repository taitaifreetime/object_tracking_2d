...
ObstacleDetector::ObstacleDetector() : Node("object_tracking_2d"), laser_scan_filter_(30.0, 0.06, 180.0, -180.0, 3)
{   
    ...
    range_max_config.description = "max distance from lasescan frame to obstacle to be detected";
    ...
    range_min_config.description = "min distance from sensor frame to obstacle to be detected";
    ...
    angle_max_config.description = "max angle in a horizontal direction to obstacle to be detected";
    ...
    angle_min_config.description = "min angle in a horizontal direction to obstacle to be detected";
    ...
    /**
     * @brief param wrt clustering obstacles
     * 
     */
    ...
    cluster_points_config.description = "clustering an obstacle";
    ...
    obstacle_center_ = cluster_points_ / 2;

    auto cluster_angle_tolerance_config = rcl_interfaces::msg::ParameterDescriptor{};
    cluster_angle_tolerance_config.description = "gap between two obstacles in a horizontal direction";
    cluster_angle_tolerance_config.additional_constraints = "highly depending on situation (e.g. large number under the situation that there are many obstacles)";
    ...
    cluster_dist_tolerance_config.description = "gap between two obstacles in a radial direction";
    cluster_dist_tolerance_config.additional_constraints = "highly depending on situation (e.g. large number under the situation that there are many obstacles)";
    ...
    laser_scan_subscriber_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "scan", qos_profile_lidar, std::bind(&ObstacleDetector::LaserScanCallback, this, std::placeholders::_1));

    // obstacle detection qos
    rmw_qos_profile_t custom_qos_profile_detection = rmw_qos_profile_default;
    custom_qos_profile_detection.history = rmw_qos_history_policy_t::RMW_QOS_POLICY_HISTORY_KEEP_LAST;
    custom_qos_profile_detection.reliability = rmw_qos_reliability_policy_t::RMW_QOS_POLICY_RELIABILITY_RELIABLE;
    custom_qos_profile_detection.durability = rmw_qos_durability_policy_t::RMW_QOS_POLICY_DURABILITY_VOLATILE;
    rclcpp::QoS qos_profile_detection(rclcpp::KeepLast(5), custom_qos_profile_detection);
    static_obs_posi_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "static_objects/position", qos_profile_detection);
    static_obs_posi_cov_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "static_objects/position/covariance", qos_profile_detection);
    static_obs_vel_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "static_objects/velocity", qos_profile_detection);
    static_obs_traj_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "static_objects/trajectory", qos_profile_detection);
    dynamic_obs_posi_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "dynamic_objects/position", qos_profile_detection);
    dynamic_obs_next_posi_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "dynamic_objects/next_position", qos_profile_detection);
    dynamic_obs_posi_cov_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "dynamic_objects/position/covariance", qos_profile_detection);
    dynamic_obs_vel_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "dynamic_objects/velocity", qos_profile_detection);
    dynamic_obs_traj_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "dynamic_objects/trajectory", qos_profile_detection);

    timer_ = this->create_wall_timer(
        std::chrono::milliseconds(1000/this->get_parameter("frequency").as_int()),
        std::bind(&ObstacleDetector::DetectionCallback, this)
    );
...
void ObstacleDetector::initializeKFSystem()
{
    /**
    ...
    kf_param_config = rcl_interfaces::msg::ParameterDescriptor{};
    kf_param_config.description = "frame out if obstacle has not been corrected for over this";
    this->declare_parameter("frames_limit", 1, kf_param_config);
    frames_limit_ = this->get_parameter("frames_limit").as_int();
    kf_param_config = rcl_interfaces::msg::ParameterDescriptor{};
    kf_param_config.description = "untracked if obstacle velocity is higher than this";
    ...
    kf_param_config = rcl_interfaces::msg::ParameterDescriptor{};
    kf_param_config.description = "velocity criteria to determine dynamic or static obstacle";
...
void ObstacleDetector::LaserScanCallback(const sensor_msgs::msg::LaserScan &msg){scan_ = msg;}

void ObstacleDetector::DetectionCallback()
{
    // laser scan pre-process
    std::vector<LaserScanFilter::PreProcScan> preproc_scans;
    laser_scan_filter_.PreProc(scan_, preproc_scans);

    // compute local minimums, means detect obstacle candidates
    std::vector<LaserScanFilter::PreProcScan> local_minimums = computeLocalMinimums(scan_, preproc_scans);
    RCLCPP_INFO(get_logger(), "Obstacle candidates: %d", local_minimums.size());
    ...
    if (obstacles_.size() > 0)
    {
        ...
        * if any tracked obstacles, kalman filter prediction first.
        ...
        */
        ...
        {
            obstacles_.at(i)->predict(
                (cur_stamp-prev_stamp_).nanoseconds()/1000000000.0, 
                control
            );
            RCLCPP_WARN(get_logger(), "\tid%d predicted. vel %.3lf", obstacles_.at(i)->id(), obstacles_.at(i)->velocity().norm());
            

            // compute cost
            int len_traj = obstacles_.at(i)->trajectory().size();
            std::vector<double> cost_row;
            for (int j=0; j<observations.size(); j++)
            {
                double eucl_dist = obstacles_.at(i)->computeEuclDistance(observations[j]);
                double squared_maha_dist = obstacles_.at(i)->computeSquaredMahaDistance(observations[j]);
                double weight = (
                    ...
                    * (len_traj > 30 ? 1.0 : 2.0)  // existing track with longer trajectory should be easilr associated
                    // * (obstacles_.at(i)->isStillTentative() ? 2.0 : 1.0)
                );
        ...
        * according to assignment result, existing tracked obstacles are associated with the observation.
        ...
        */
        ...
        for (int i=0; i<observations.size(); i++)
        {
            ...
            try{
                if (obstacles_.at(index)->computeEuclDistance(observations.at(i))<matching_dist_)
                {
                    obstacles_.at(index)->correct(
                        observations.at(i), 
                        cur_time, 
                        frames_sta2dyn_, frames_dyn2sta_,  
                        velocity_sta2dyn_
                    );
                    RCLCPP_WARN(get_logger(), "\tid%d corrected. vel %.3lf", obstacles_.at(index)->id(), obstacles_.at(index)->velocity().norm());
                }
        ...
        RCLCPP_INFO(get_logger(), "Delete obstacles");
        for (int i=0; i<obstacles_.size();)
        {
            if (obstacles_.at(i)->isStillTentative() ||
                obstacles_.at(i)->isUncorrectedOverThan(frames_limit_) || 
                obstacles_.at(i)->isHigherSpeedThan(velocity_limit_) )
            {
                RCLCPP_WARN(get_logger(), "\tid%d deleted.", obstacles_.at(i)->id());
                existing_ids_.erase(obstacles_.at(i)->id());
    ...
    /**
     * @brief new obstacles join
     * unmatched observations are assigned to new obstacles to be tracked
     */
    ...
    for (int id : unmatched_observation_id)
    {
        try
        {
            ...
            observation << 
                ...
                0.0, 
                0.0;
            obstacles_.push_back(std::make_shared<Obstacle>(
    ...
    publishObstacles(cur_stamp);
    
    prev_stamp_ = cur_stamp;
}

std::vector<LaserScanFilter::PreProcScan> ObstacleDetector::computeLocalMinimums(
...
) const
{
    ...
    for (int i=obstacle_center_; i<num_preproc_scans-obstacle_center_; i++)
    {
        if (preproc_scans[i-obstacle_center_].range > preproc_scans[i].range // explore local minimum from left
            && preproc_scans[i].range < preproc_scans[i+obstacle_center_].range) // explore local minimum from right
        {
            LaserScanFilter::PreProcScan min_scan=preproc_scans[i];
            for (int j=i+1; j<i+obstacle_center_; j++)
            {
...
void ObstacleDetector::publishObstacles(const rclcpp::Time &stamp) const
{
    // color
    ...
    {
        if (obstacles_[i]->isStillTentative()) continue;

        if (obstacles_[i]->isDynamic())
        {
            // add position & text marker
            marker.id = obstacles_[i]->id()*2; // position marker array contains position & id visualization 
            marker.pose.position.x = obstacles_[i]->position()[0];
            marker.pose.position.y = obstacles_[i]->position()[1];
            ...
            marker_id_text.pose.position.x = marker.pose.position.x;
            marker_id_text.pose.position.y = marker.pose.position.y;
            marker_id_text.text = std::to_string(obstacles_[i]->id());
            dyn_markers.markers.push_back(marker_id_text);

            // add velocity marker
            marker_vel.id = marker.id;
            marker_vel.pose.position = marker.pose.position;
            Eigen::Vector2f vel = obstacles_[i]->velocity();
            tf2::Quaternion q;
            q.setRPY(0, 0, atan2(vel[1], vel[0]));
            marker_vel.pose.orientation = tf2::toMsg(q);
            marker_vel.scale.x = std::max(static_cast<double>(vel.norm()), 0.0000001);
            ...
            {
                marker.color.a = 0.5;
                marker.pose.position.x += time_horizon_ * vel[0];
                marker.pose.position.y += time_horizon_ * vel[1];
            ...
            {
                Eigen::Matrix2f covariance = obstacles_[i]->covariance().block(0,0, 2,2); // get covariance wrt position
                ...
                marker_pos_cov.id = obstacles_[i]->id();
    ...
    static_obs_posi_publisher_->publish(markers);
    static_obs_vel_publisher_->publish(markers_vel);
    dynamic_obs_posi_publisher_->publish(dyn_markers);
    dynamic_obs_vel_publisher_->publish(dyn_markers_vel);
    if (visualize_next_position_) dynamic_obs_next_posi_publisher_->publish(dyn_next_markers);
    if (visualize_covariance_) 
    {
        static_obs_posi_cov_publisher_->publish(markers_pos_cov);
        dynamic_obs_posi_cov_publisher_->publish(dyn_markers_pos_cov);
    }
    if (visualize_trajectory_) 
    {
        static_obs_traj_publisher_->publish(markers_traj);
        dynamic_obs_traj_publisher_->publish(dyn_markers_traj);
    }
